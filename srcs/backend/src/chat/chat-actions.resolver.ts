...
export class ChatActionsResolver {
  ...
  async canPunish(
    ...
    const punisherIsAdmin = await this.chatService.isChatAdmin(
      chat.id,
      punisherId,
    );
  ...
  async addUserPunishmentInChat(
    ...
    @Args('targetUserId', { type: () => Int }, ParseIntPipe)
    targetUserId: number,
    ...
    await this.chatService.addPunishment(
      chat.id,
      user.id,
      degree,
      targetUserId,
  ...
  async removeUserPunishmentInChat(
    ...
    await this.chatService.removePunishmentByParams(
      chat.id,
      degree,
      targetUserId,
    );
  ...
  async addMemberToChat(
    ...
    if (chat.is_private || chat.password?.length) {
      const isAdmin =
        userId && (await this.chatService.isChatAdmin(chatId, user.id));
    ...
    const bannedPunishments = await this.chatService.getPunishments(
      chat.id,
      PunishmentDegree.BAN,
    );
    const isBanned = bannedPunishments.find((p) => p.toUserId == user.id);
    if (isBanned) throw new UnauthorizedException('You banned');

    await this.chatService.addToChat(chat.id, userId || user.id);
    return true;
  }
  ...
  async removeMemberFromChat(
    ...
    @Args('userId', {
      nullable: true,
      type: () => Int,
      description: 'default: current user',
    })
    userId?: number,
    @CurrentUser() user?: RequestUser,
  ) {
